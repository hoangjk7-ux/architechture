...
const healthConfig: Record<HealthStatus, { color: string; dot: string; key: string }> = {
  healthy: { color: "text-green-400", dot: "bg-green-400", key: "health.healthy" },
  degraded: { color: "text-yellow-400", dot: "bg-yellow-400", key: "health.degraded" },
  down: { color: "text-red-400", dot: "bg-red-400", key: "health.down" },
  unknown: { color: "text-gray-400", dot: "bg-gray-400", key: "health.unknown" },
};
...
function IntegrationForm({ initial, onSave, onClose }: {
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.sourceSystemId || !form.destinationSystemId) {
      toast.error(t("integrations.toast.validation")); return;
    }
    setSaving(true);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2 space-y-1">
          ...
          <Input value={form.name} onChange={(e) => set("name", e.target.value)} placeholder={t("integrations.form.namePlaceholder")} className="bg-input" />
        </div>
        <div className="space-y-1">
          ...
          <Select value={form.sourceSystemId} onValueChange={(v) => set("sourceSystemId", v as Id<"software_systems">)}>
            <SelectTrigger className="bg-input"><SelectValue placeholder={t("integrations.form.selectSource")} /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-1">
          ...
          <Select value={form.destinationSystemId} onValueChange={(v) => set("destinationSystemId", v as Id<"software_systems">)}>
            <SelectTrigger className="bg-input"><SelectValue placeholder={t("integrations.form.selectDestination")} /></SelectTrigger>
            <SelectContent>
              {systems.map((s) => <SelectItem key={s._id} value={s._id}>{s.name}</SelectItem>)}
            </SelectContent>
          </Select>
        ...
        <div className="space-y-1">
          ...
          <Select value={form.method} onValueChange={(v) => set("method", v as typeof form.method)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="event_driven">{t("method.event_driven")}</SelectItem>
        ...
        <div className="space-y-1">
          ...
          <Input value={form.owner} onChange={(e) => set("owner", e.target.value)} placeholder={t("integrations.form.ownerPlaceholder")} className="bg-input" />
        </div>
        <div className="space-y-1">
        ...
        <div className="col-span-2 space-y-1">
          ...
          <Textarea value={form.description} onChange={(e) => set("description", e.target.value)} placeholder={t("integrations.form.descriptionPlaceholder")} className="bg-input" rows={2} />
        </div>
      </div>
...
function IntegrationsContent() {
  ...
  const handleCreate = async (data: IntegrationFormData) => { await createIntegration(data); toast.success(t("integrations.toast.added")); };
  const handleUpdate = async (data: IntegrationFormData) => {
    if (!editing) return;
    await updateIntegration({ id: editing._id, ...data });
    toast.success(t("integrations.toast.updated")); setEditing(null);
  };
    ...
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">{integrations.length} {t("integrations.subtitleSuffix")}</p>
        </div>
        {canWrite && (
          <Button onClick={() => setShowForm(true)} size="sm" className="gap-2">
            <Plus className="h-4 w-4" />{t("integrations.addIntegration")}
          </Button>
        )}
      </div>
            ...
            return (
              <div key={s} className="bg-card border border-border rounded-lg p-3 flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">{t(cfg.key)}</div>
                </div>
              </div>
            );
          })}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-accent/50">
                <th className="text-left p-3 font-medium text-muted-foreground">{t("integrations.col.integration")}</th>
                <th className="text-left p-3 font-medium text-muted-foreground">{t("integrations.col.sourceDestination")}</th>
                <th className="text-left p-3 font-medium text-muted-foreground hidden md:table-cell">{t("integrations.col.protocol")}</th>
                <th className="text-left p-3 font-medium text-muted-foreground">{t("integrations.col.health")}</th>
                <th className="text-left p-3 font-medium text-muted-foreground hidden lg:table-cell">{t("integrations.col.compliance")}</th>
                <th className="text-right p-3 font-medium text-muted-foreground">{t("integrations.col.actions")}</th>
              </tr>
            </thead>
            <tbody>
              {integrations.map((i) => {
                  ...
                  <tr key={i._id} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={cn("text-xs", health.color)}>{t(health.key)}</span>
                      </div>
                    </td>
                    <td className="p-3 hidden lg:table-cell">
                      {i.isArchitectureCompliant
                        ? <span className="text-green-400 text-xs flex items-center gap-1"><CheckCircle className="h-3 w-3" />{t("integrations.compliant")}</span>
                        : <span className="text-red-400 text-xs flex items-center gap-1"><XCircle className="h-3 w-3" />{t("detail.nonCompliant")}</span>
                      }
                    </td>
                    <td className="p-3 text-right">
                      {canWrite && (
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button variant="ghost" size="icon" className="h-7 w-7 cursor-pointer text-destructive hover:text-destructive" onClick={() => { removeIntegration({ id: i._id }); toast.success(t("integrations.toast.removed")); }}><Trash2 className="h-3 w-3" /></Button>
                        </div>
                      )}
                    </td>
                  </tr>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-xl bg-card border-border">
          <DialogHeader><DialogTitle>{t("integrations.addIntegration")}</DialogTitle></DialogHeader>
          <IntegrationForm onSave={handleCreate} onClose={() => setShowForm(false)} />
        </DialogContent>
      </Dialog>
      <Dialog open={!!editing} onOpenChange={(open) => !open && setEditing(null)}>
        <DialogContent className="max-w-xl bg-card border-border">
          <DialogHeader><DialogTitle>{t("integrations.editIntegration")}</DialogTitle></DialogHeader>

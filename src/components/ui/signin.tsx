    ...
    <div className={cn("w-full space-y-4", className)} {...props}>
      <div className="space-y-5 rounded-2xl border border-border/70 bg-card/80 p-6 shadow-xl shadow-black/20 backdrop-blur-sm">
        <div className="space-y-1 text-center">
          <p className="text-base font-semibold text-foreground">{signInText ?? t("auth.signIn")}</p>
        ...
        <Button type="button" variant="outline" className="w-full" disabled={isSubmitting} onClick={beginGoogleSignIn}>
          {isSubmitting ? "…" : language === "vi" ? "Tiếp tục với Google" : "Continue with Google"}
        </Button>
      </div>
    </div>
  );
